...
这是一个 Codex 的自定义状态栏脚本项目，用 Bash 编写。脚本从 stdin 读取 JSON 输入，解析后输出一段带 ANSI 颜色的紧凑状态行。

## 项目结构

```
statusline.sh   ← 核心脚本，Codex 通过 statusLine 设置调用
install.sh      ← 安装器，复制脚本到 ~/.Codex/ 并修改 settings.json
examples/
  sample-input.json           ← statusline.sh 的典型输入样例
  statusline.settings.json    ← Codex settings.json 中需要的配置片段
```

## 数据流

1. Codex 调用 `statusLine.command` 指向的脚本
2. Codex 通过 stdin 传入当前会话状态的 JSON
...
# 用 echo 传入自定义 JSON
echo '{"model":{"display_name":"Codex-sonnet-4-6"},"context_window":{"used_percentage":85}}' | ./statusline.sh
